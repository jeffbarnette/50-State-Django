...
class TestModels(TestCase):
    ...
    def test_error_when_state_name_is_null(self):
        ...
        self.assertIn('null value in column "name"', error_message[0])
    ...
    def test_error_when_state_abbreviation_is_null(self):
        ...
        self.assertIn('null value in column "abbr"', error_message[0])
    ...
    def test_error_when_capital_is_null(self):
        ...
        self.assertIn('null value in column "capital_id"', error_message[0])
...
class TestView(TestCase):
    ...
    def test_state_list_view_returns_correct_state_and_capital_pairing(self):
        """Test that state list view returns correct capital for each state"""
        response = self.client.get(reverse('states'))
        self.assertEqual(response.status_code, 200)
        ...
        # Check that each state has the correct capital
        valid_source_pairs = {
            k: v for k, v in zip(self.valid_states, self.valid_capitals)
        }
        valid_response_pairs = {
            k: v for k, v in zip(response_state_list, response_capital_list)
        }
        self.assertEqual(valid_response_pairs, valid_source_pairs)
